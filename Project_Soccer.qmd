...
```{r}
# Modified create_unified_dataset function with better column checking
create_unified_dataset <- function() {
  # First check what minutes columns are actually available
  minutes_cols <- grep("Min|MP", colnames(player_standard_stats), value = TRUE)
  cat("Available minutes columns:", paste(minutes_cols, collapse = ", "), "\n")
  
  # Add standardized minutes played column
  player_standard_stats <- player_standard_stats %>%
    mutate(
      # Check which minutes column exists and use it appropriately
      Minutes_Played = case_when(
        "Min_Playing" %in% colnames(player_standard_stats) ~ Min_Playing,
        "MP_Playing" %in% colnames(player_standard_stats) ~ MP_Playing * 90, # Multiply games by 90 minutes
        "Mins_Per_90_Playing" %in% colnames(player_standard_stats) ~ Mins_Per_90_Playing * 90,
        TRUE ~ 0
      )
    )
  
  # Join all datasets using consistent keys
  unified_data <- player_standard_stats %>%
    # Join with shooting stats
    left_join(
      player_shooting_stats %>%
        select(Player, Squad, Season_End_Year, 
               any_of(c(contains("Sh"), contains("Gls"), contains("xG")))),
      by = c("Player", "Squad", "Season_End_Year")
    ) %>%
    # Join with passing stats
    left_join(
      player_passing_stats %>%
        select(Player, Squad, Season_End_Year,
               any_of(c(contains("Cmp"), contains("Att"), contains("xA"), "KP"))),
      by = c("Player", "Squad", "Season_End_Year")
    ) %>%
    # Join with defensive stats
    left_join(
    ...
    left_join(
      player_possession_stats %>%
        select(Player, Squad, Season_End_Year,
               any_of(c(contains("Touches"), contains("Carries"), contains("Prog")))),
      by = c("Player", "Squad", "Season_End_Year")
    )
  
  # Check if goalkeeper data exists and join if available
  if (exists("player_gk_stats") && !is.null(player_gk_stats) && nrow(player_gk_stats) > 0) {
    # Identify goalkeeper stat columns that actually exist
    gk_cols <- c("Player", "Squad", "Season_End_Year")
    
    # Add save columns if they exist
    possible_save_cols <- c("Save_percent", "Saves", "PSxG", "GA")
    existing_save_cols <- intersect(possible_save_cols, colnames(player_gk_stats))
    
    if (length(existing_save_cols) > 0) {
      unified_data <- unified_data %>%
        left_join(
          player_gk_stats %>%
            select(Player, Squad, Season_End_Year, all_of(existing_save_cols)),
          by = c("Player", "Squad", "Season_End_Year")
        )
    }
  }
  ...
  # Check for success
  if (!"Minutes_Played" %in% colnames(unified_data) || all(unified_data$Minutes_Played == 0)) {
    warning("Minutes_Played calculation may have failed. Check column names.")
    
    # As a fallback, if we failed to get Minutes_Played but we have MP_Playing
    if ("MP_Playing" %in% colnames(unified_data)) {
      cat("Using MP_Playing as fallback for minutes calculation\n")
      unified_data <- unified_data %>%
        mutate(Minutes_Played = MP_Playing * 90)
    }
  }
