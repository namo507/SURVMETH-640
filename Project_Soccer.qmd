...
extract_season_data <- function(season_year) {
  ...
  names(table_data) <- c("position", "club", "matches", "wins", "draws", "losses", 
  ...
  transfer_page <- read_html(transfer_url)
  
  # Extract club expenditure data
  transfer_data <- transfer_page %>%
  ...
  for(i in seq_along(transfer_data)) {
    if(i %% 2 == 1) { # Club transfer tables are at odd indices
      club_name <- transfer_page %>%
        html_nodes(".large-8 .box h2") %>%
        .[ceiling(i/2)] %>%
        html_text() %>%
        trimws()
      ...
      if(length(transfer_data[[i]]) > 0) {
        ...
        exp_text <- transfer_data[[i]][exp_row, exp_col]
        
        # Parse the expenditure value (e.g., "€10.50m" to 10.5)
